...
import { v4 } from 'uuid';
import { Ref } from 'vue';
import { MakeOptional, Scalars } from '../generated/graphql';
...
export function useUpdate<Model extends ModelTemplate>(
  useUpdateMutation: (
    options: UseMutationOptions<
      UpdateMutationResult<Model>,
  ...
  async function update(
    id: MongoId,
    modifiedFields: ModifiableFields<Model>,
  ): Promise<Model> {
    const { data } = await mutate(
      {
        id,
...
>(
  useDestroyMutation: (
    options: UseMutationOptions<DestroyMutationResult<Model>, DestroyVariables>,
      ...
      },
      {
        update(cache, result) {
          ...
          cache.writeQuery({
            ...
            data: {
              ...cachedQuery,
              records: cachedQuery.records.filter(
                ({ _id }) => _id !== deletedId,
              ),
            },
          });
